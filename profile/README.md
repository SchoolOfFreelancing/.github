<h1 align="center">School of Freelancing</h1>
<h2 align="center">Built for Freelancers, by Freelancers</h2>

<p align="center">
  <b>Linux, VoIP, SMS, DevOps, Cybersecurity & AI Skills Training — from Comilla, Bangladesh to the Global Marketplace</b>
</p>

<p align="center">
  <a href="https://schooloffreelancing.com">🌐 Website</a> •
  <a href="#-training-repositories">📚 Training</a> •
  <a href="#-tech-support-repositories">🛠 Tech Support</a> •
  <a href="#-lets-talk">📩 Let's Talk</a>
</p>

## About

**School Of Freelancing** — Linux Training, AI Training & Linux System Administration Support for Freelancers and Businesses Worldwide. Learn Linux, Ubuntu Server, cloud computing, DevOps, Docker, cybersecurity, AI tools, self-hosted AI, and AI infrastructure through practical, real-world training and technical support. Powered by Free and Open Source Software (FOSS) since 2015. Led by Masum, a Bangladesh Government-Verified Freelancer (F126196700864@freelancers.gov.bd) and expert Linux System Administrator — learn from someone who's proven in the field, not just the classroom.

We help beginners develop Linux and AI skills for freelancing, build practical IT expertise, and pursue opportunities in the global freelance market. We also help businesses with Linux server support, VPS support, cloud infrastructure, AI setup, server troubleshooting, DevOps, and technical solutions.

Our mission is to democratize Linux and AI education worldwide, helping people turn technical skills into freelance opportunities while helping businesses maintain reliable IT infrastructure.

Learn Linux. Master AI. Build Freelance Skills. Solve Technical Problems. Work Globally.

Two audiences, one platform:

- 🎓 **Beginner freelancers** — On-demand hands-on training ending in a Linux freelancing career on worldwide marketplaces.
- 🏢 **Business Clients** — Linux, VoIP, and cloud infrastructure support for business or individual needs.

## 📚 Training Repositories

| Repositories | Focus |
|---|---|
| Linux Server Training | Ubuntu / CentOS / AlmaLinux administration |
| AI Agent Mastery | Hermes Agent, Odysseus AI Workspace, VAPI, OpenClaw, LocalAI, ZeroClaw |
| DevOps Mastery | GitLab CE, GitHub Actions, GHES |
| VoIP Mastery | Telnyx + VAPI, Twilio + VAPI, FusionPBX, GoAutoDial |
| SMS Mastery | Telnyx SMS API + Twilio SMS API + VAPI, FusionPBX, GoAutoDial |
| Cybersecurity Training | On-demand training to kick-start your cybersecurity learning |

On-demand hands-on Linux training on cutting-edge tech & trends.

## 🛠 Tech Support Repositories

- Linux Server Services
- Hermes Agent Support
- AI Agent Support
- DigitalOcean Cloud Support
- Vapi Platform Support
- Hummingbot Installation Support
- GitLab Community Edition Setup
- GitHub Enterprise Server Setup
- FusionPBX VoIP Support
- OpenAI Platform Support
- Claude Platform Support
- Twilio SMS API & Telnyx SMS REST API Setup

## 📩 Let's Talk

- WhatsApp: [WhatsApp](https://wa.me/8801748973769)
- Founder: Masum

---

<p align="center">© School of Freelancing. All rights reserved. Built with ❤️ from Bangladesh.</p>
